export default {
  ...
  articles: {
    sectionLabel: '出版物',
    sectionTitle: '技術 ',
    sectionTitleHighlight: '出版物',
    sectionDescription: 'アーキテクチャ、AI可読性、スケーラブルシステムに関する記事とケーススタディ。',
  },
  article: {
    ctaLabel: '他の記事を読みたいですか？',
    machineReadableEngineer: {
      metaTitle: '機械可読なエンジニア | Kaio Morsch',
      metaDescription: 'JSON-LD、許容的なクローリング、セマンティックメタデータを使用して、AIエージェントとATS向けにポートフォリオを最適化した方法。',
      badge: '記事',
      readTime: '読了時間: 4分',
      title: '機械可読なエンジニア: AIエージェントとATS向けにポートフォリオを最適化した方法',
      author: 'Kaio Vítor Morsch 著',
      introParagraphs: [
        '長年にわたり、私たちは人間の目のためにウェブを構築してきました。ピクセルパーフェクトなレイアウト、CSSアニメーション、最適なフォントサイズについて議論しました。しかし、採用と情報発見の環境は変化しました。今日、ポートフォリオの最初の訪問者が採用担当者であることは稀です — それは採用管理システム（ATS）、AIクローラー、または自動化されたソースエージェントです。',
        'ポートフォリオが標準的なHTMLタグのみに依存して経験を伝えている場合、パースアルゴリズムとの危険な賭けをしていることになります。私は推測をやめ、機械と明示的に通信し始めることにしました。ここでは、ポートフォリオを完全に機械可読にするために最適化した方法を紹介します。',
      ],
      sections: [
        {
          label: '01 // スキーマ注入',
          title: '推測をやめ、宣言を始める: JSON-LDの力',
          paragraphs: [
            'AIエージェントや古いATSがウェブページをパースする際、DOM構造に依存することは賭けです。そのh3は職名を表しているのでしょうか、それともプロジェクト名でしょうか？',
            'パースエラーや幻覚を排除するために、私はHTMLスクレイピングを完全に回避し、useSchemaOrgProfileコンポーザブルを介してJSON-LDを使用した厳格なSchema.orgマークアップを実装しました。ボットにUIをスクレイピングさせるのではなく、構造化されたPersonオブジェクトを直接提供します。技術スタックはknowsAbout配列に、職歴はworksForに、学歴はalumniOfに明示的に記述されています。',
          ],
        },
        {
          label: '02 // クローリングポリシー',
          title: '扉を開く: 許容的なrobots.txt',
          paragraphs: [
            'AIアクセシビリティは正面の扉から始まります。最近、帯域幅を節約したりコンテンツを保護したりするためにAIクローラーをブロックする開発者の傾向が見られます。',
            'プロフェッショナルなポートフォリオにとって、これは逆効果です。私のrobots.txtは意図的に許容的です（User-Agent: * にdisallowルールなし）。自動化された採用エージェントがテックリード向けに私の経歴を要約しようとしている場合、私の職歴全体をインデックスするための無制限のアクセスを保証したいと考えています。',
          ],
        },
        {
          label: '03 // グローバルコンテキスト',
          title: 'グローバルコンテキスト: メタデータとネイティブi18n',
          paragraphs: [
            'LLMはグローバルに動作し、労働市場も同様です。AIエージェントに正しいコンテキストを提供することで、処理時間を即座に節約し、精度を確保できます。',
            'アプリケーション設定を通じて、即時のコンテキストを確立するためのクリーンなメタデータを構造化しました。さらに、私の国際化戦略は、ボットに正しい言語をネイティブに提供するように構築されています。米国の採用担当者がAIツールを使用して候補者を探している場合、ボットはページを動的に翻訳して技術用語を失うリスクを負うことなく — 英語のローカリゼーションに直接アクセスします。',
          ],
        },
        {
          label: '04 // 総括',
          title: '新しいウェブのために構築する',
          paragraphs: [
            '生成AIの時代において、私たちのターゲットオーディエンスはもはやサイトを閲覧する人間だけではありません。私たちは、大規模にクロール、要約、評価を行うインテリジェントエージェントのために開発しています。',
            '許容的なクローリング、セマンティックメタデータ、厳格なJSON-LDスキーマを組み合わせることで、私たちのデータがボットが期待する場所に正確にあることを保証します。ウェブの未来は機械可読であり、エンジニアとして、私たちの個人的なインフラストラクチャはその現実を反映すべきです。',
          ],
        },
      ],
    },
